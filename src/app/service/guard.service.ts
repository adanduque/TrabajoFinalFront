...
export class GuardService implements CanActivate {
  ...
  canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot) {
    ...
    if (!helper.isTokenExpired(token)) {
      let url = state.url;
      //IF LA RUTA ES PROFILE SOLO VERIFICAR QUE ESTE AUTENTICADO
      if(url.startsWith("/pages/profile")){
        return true;
      }
      ...
      return this.menuService.getMenusByUser(decodedToken.user_name).pipe(map( (data: Menu[]) => {
